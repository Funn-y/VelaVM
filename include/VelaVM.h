...
class VelaVM
{
    std::vector<unsigned char> code_raw;
...
public:
    explicit VelaVM(std::vector<unsigned char> raw)
        : code_raw(std::move(raw))
    {
    ...
    int run()
    {
        if (code_raw.empty()) return -1;
        size_t index = 0;
        while(!stop && index < code_raw.size())
        {
            run_command(index);
            index++;
        }
